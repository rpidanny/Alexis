...
const char HTML_DEVICES_SCRIPT[] PROGMEM  = "<script>function onLoad(){var closebtns = document.getElementsByClassName(\"close\"); var i; for (i = 0; i < closebtns.length; i++) { closebtns[i].addEventListener(\"click\", function(e) { var r = new XMLHttpRequest();r.open( \"GET\", \"/del?name=\"+e.path[1].childNodes[0].outerText, false );r.send(null);window.location.href = \"/devices\";});}}</script>";
const char HTML_HEAD_END[] PROGMEM        = "</head><body onload=\"onLoad()\"><div style='display: inline-block;min-width: 260px;border: 1px solid gray;padding: 20px;padding-bottom: 35px;padding-top: 10px; box-shadow: 3px 3px 10px 0px rgba(0,0,0,0.46);background-color: #fff;'>";
